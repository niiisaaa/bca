...
<div class="page-header header-filter header-small" data-parallax="true"
     style="background-image: url('./dress/dark-bg.jpg');">
    <div class="container">
        <div class="row">
            <div class="col-md-8 ml-auto mr-auto text-center">
                <div class="brand">
                    <h1 class="title">Products</h1>
                    <h4>Where Fashion Meets Occasion - Trendy to Traditional</h4>
                </div>
            </div>
        </div>
    </div>
...
<div class="row">
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            $prod_id = htmlspecialchars($row['prod_id']);
            $prod_desc = htmlspecialchars($row['prod_desc']);
            $prod_title = htmlspecialchars($row['prod_title']);
            $prod_image = htmlspecialchars($row['prod_images']);
            $prod_brand = htmlspecialchars($row['prod_brand']);
            $prod_category = htmlspecialchars($row['prod_category']);
            $prod_price = htmlspecialchars($row['prod_price']);
            $prod_stock = htmlspecialchars($row['prod_stock']);
                ...
                <div class="card card-product card-plain">
                    <div class="card-header card-header-image">
                        <a href="view_product.php?prod_id=' . $prod_id . '">
                            <img src="./' . (!empty($prod_image) ? $prod_image : 'default-image.jpg') . '" alt="Product Image" style=" object-fit:content;width:50%"; />
                        </a>
                    </div>
                    ...
                    <div class="card-footer">
                        <div class="price-container">
                            <span class="price price-new">Rs. ' . $prod_price . '</span>
                        </div>
                        <div class="ml-auto">
                            <button type="button" onclick="addToCart(' . $prod_id . ', ' . $_SESSION['id'] . ')" 
...
<script>
    ...
    $(document).ready(function () {
        ...
        slider.noUiSlider.on('update', function (values, handle) {
            if (handle) {
                limitFieldMax.innerHTML = "Rs. " + Math.round(values[handle]);
            } else {
                limitFieldMin.innerHTML = "Rs. " + Math.round(values[handle]);
            }
        });
    });
